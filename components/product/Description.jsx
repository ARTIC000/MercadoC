...
const Description = props => {
  ...
  return (
    <Pressable>
      <View className="px-3">
        <Text className="mb-3 lg:border-b-2 lg:border-red-500 w-fit">introducir</Text>
        <Text className="text-sm leading-6 tracking-wider text-gray-600 lg:text-sm lg:leading-8">
          {isShowDesc ? description : truncate(description, 300)}p
        </Text>
        {description.length > 300 && (
          ...
          >
            {isShowDesc ? (
              <Text className="text-sm text-sky-400">cerca</Text>
            ) : (
              <Text className="text-sm text-sky-400">Ver más</Text>
            )}
            {!isShowDesc && (
